...
public class BatteryInfoPlugin extends Plugin {
    ...
    private String BatteryHistoryTitle = "";
    ...
    private void genBatteryInfo(BugReportModule br, Chapter ch) {
        ...
        if (sec == null) {
            br.printErr(3, TAG + "Section not found: " + Section.DUMP_OF_SERVICE_BATTERYINFO + " or " + Section.DUMP_OF_SERVICE_BATTERYSTATUS + " (ignoring it)");
            return;
        }
        ...
        boolean OldBatteryHistory = false;
        int eventS = 0;
        int battery_levelS = 0;
        long last_batteryhistory_time = 0;
        long ts = 0;
        while (idx < cnt) {
            String buff = sec.getLine(idx++);
            if (buff.equals("Battery History:")) {
            	OldBatteryHistory = true;
                foundBatteryHistory = true;
                BatteryHistoryTitle = buff;
                battery_levelS = 22;
                eventS = 35;
                idx_temp = idx;
                break;
            } else if (buff.startsWith("Battery History (")) {
            	OldBatteryHistory = false;
            	foundBatteryHistory = true;
            	BatteryHistoryTitle = buff;
            	battery_levelS = 26;	//ignore " (*)" label
            	eventS = 39;
            	idx_temp = idx;
            	br.printErr(3, TAG + "======================idx_temp is" + idx_temp);
            	
            }
            if (buff.length() == 0) {
            	br.printErr(3, TAG + "======================idx is" + idx);
            	br.printErr(3, TAG + "======================sec.getLine(idx-2).substring(0, 21)is " + sec.getLine(idx-2).substring(0, 21));
            	last_batteryhistory_time = Util.parseRelativeTimestamp(sec.getLine(idx-2).substring(0, 21));
            	idx = idx_temp;
            	break;
            }  
        }
        ...
        if (!foundBatteryHistory) {
            ...
            ChartGenerator chart = new ChartGenerator(BatteryHistoryTitle);
            mDatas = new HashMap<String, DataSet>();
            mConn = new HashSet<String>();
            
            DataSet levelDs = new DataSet(Type.PLOT, "Battery level", Color.blue.getRGB());
            DataSet voltDs = new DataSet(Type.PLOT, "Battery volt", Color.green.getRGB());
            DataSet tempDs = new DataSet(Type.PLOT, "Battery temp", Color.red.getRGB());
//            levelDs.setMin(0);
//            levelDs.setMax(100);
            levelDs.setAxisId(1);
            voltDs.setAxisId(2);
            ...
            while (idx < cnt) {
                ...
                // Read the timestamp
                if (OldBatteryHistory)
                	ts = ref - Util.parseRelativeTimestamp(buff.substring(0, 21));
                else	//parsering time tramp in new Android version L 
                	ts = ref - (last_batteryhistory_time - Util.parseRelativeTimestamp(buff.substring(0, 21)));

                // Read the battery level
                String levelS = buff.substring(battery_levelS, battery_levelS+3);
                if (levelS.charAt(0) == ' ') continue; // there is a disturbance in the force...
                if (levelS.contains("S") || levelS.contains("T")) continue;
                int level = Integer.parseInt(levelS);
                levelDs.addData(new Data(ts, level));

                // Parse the signal levels
                if (buff.length() > eventS) {
                    buff = buff.substring(eventS);
                    ...
                    for (String s : signals) {
                        ...
                        if (c == '+') {
                            ...
                            if (eq > 0) {
                                ...
                                if (s.equals("volt"))
                                	voltDs.addData(new Data(ts, Integer.parseInt(value)));
                                if (s.equals("temp"))
                                	tempDs.addData(new Data(ts, Integer.parseInt(value)));
                            }
                        }
                    }
                }
    ...
    private void genStats(BugReportModule br, Chapter ch, Node node, boolean detectBugs, String csvPrefix) {
        ...
        for (Node item : node) {
            ...
            if (line.endsWith(":") && Util.isDigit(line.substring(line.length()-2, line.length()-1))) {
//            	br.printErr(3, TAG + "@@@@@@@@ line is: " + line + "line.substring(line.length()-2, line.length()-1)): " + (line.substring(line.length()-2, line.length()-1)));
            	if (line.startsWith("u")) continue;	//currently disable "uxxx" processors parsering
            	String sUID = null;
            	if (line.startsWith("#"))
                ...
                for (Node subNode : item) {
                    ...
                    if (s.contains("(nothing executed)")) continue;	//ignore empty UID dump
                    ...
                    } else if (s.startsWith("Network: ") || s.startsWith("Mobile network: ") || s.startsWith("Wi-Fi network: ")) {
...
//                        	br.printErr(3, TAG + "$$$$startsWith(\"Network: \") line:" +s + "\n" +m.group(1)+ "\n" +m.group(2)+ "\n" +m.group(3));
                ...
                if (!line.contains("(nothing executed)")) {
                    ...
                    pre.addln(line);
                	for (DumpTree.Node itemchild : item) {
                		pre.add("  ");
                		pre.addln(itemchild.getLine());
                	}
            }
        }
