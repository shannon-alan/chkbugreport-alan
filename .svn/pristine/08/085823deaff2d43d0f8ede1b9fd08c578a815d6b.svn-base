...
public class AlarmManagerPlugin extends Plugin {
    ...
    public void load(Module br) {
        ...
        final String nodeBatchKey = "Batch{";	//Android KK add Batch section in alarm service dump
		final String nodePastKey = "Past-due"; // Android L add Past-due section in alarm service dump

        //Tinno:CJ
    	for (DumpTree.Node rootitems : dump) {
        	if(rootitems.getLine().startsWith(nodeKey)
        			|| rootitems.getLine().startsWith(nodeBatchKey)
        			|| rootitems.getLine().startsWith(nodePastKey))
        	{            		
    	        boolean stats = false;
    	        for (DumpTree.Node item : rootitems) 
    	        {
    	            String line = item.getLine();
    	            if (stats) {
    	                addPackageStat(br, item);
    	            } else if (line.contains("Top Alarms:")) {
    	      		    TopNode = item;		
    ...
    private void addPackageStat(Module br, Node item) {
        ...
        BugReportModule brm = (BugReportModule) br;
        stat.anchor = new Anchor("a" + mNextAlarmAnchor++);
        
        //Tinno:CJ		KK	com.chaozh.iReaderFree +7s507ms running, 60 wakeups:
        //						+33s264ms 0 wakes 256 alarms: act=android.intent.action.TIME_TICK
        //Tinno:Shaobin	L	1000:android +12s728ms running, 5 wakeups:
        //						+8s995ms 0 wakes 54 alarms: *alarm*:android.intent.action.TIME_TICK
        if (brm.getAndroidVersionSdk() >= AndroidVersions.SDK_KK) {
	        Pattern p = null, act = null;
	        //parser main node about Alarm Stats:
	        if (brm.getAndroidVersionSdk() == AndroidVersions.SDK_L) {
	        	p = Pattern.compile("(.*):(.*) (.*) running, ([0-9]+) wakeups:");
	        	act = Pattern.compile("(.*) ([0-9]+) wakes ([0-9]+) alarms: (.*):(.*)");
	        	
	        }else if(brm.getAndroidVersionSdk() >= AndroidVersions.SDK_KK) {
	        	p = Pattern.compile("(.*) (.*) running, ([0-9]+) wakeups:");
	        	act = Pattern.compile("(.*) ([0-9]+) wakes ([0-9]+) alarms: act=(.*)");
	        }
	        ...
	        if (brm.getAndroidVersionSdk() == AndroidVersions.SDK_L) {
	        	...
	        	stat.pkg = m.group(1);
		        stat.runtime = Util.parseRelativeTimestamp(m.group(2));
		        stat.wakeups = Long.parseLong(m.group(3));
	        }
	        //parser detail node action info 
	        for (int i = 0; i < item.getChildCount(); i++) {
	            Node child = item.getChild(i);
	            m = act.matcher(child.getLine());
    ...
    private void addAlarm(Module br, Node item) {
        ...
        BugReportModule brm = (BugReportModule) br;
        Pattern p = null;
        //Tinno:CJ		KK 		ELAPSED #0: Alarm{43326000 type 3 android}
        //Tinno:Shaobin L		ELAPSED_WAKEUP #0: Alarm{1bc69603 type 2 when 3271636 android}
        if (brm.getAndroidVersionSdk() <= AndroidVersions.SDK_KK_MR1)
        	p = Pattern.compile("([A-Z_]+) #[0-9]+: Alarm\\{[a-f0-9]+ type [0-3] (.*)\\}");
        else if (brm.getAndroidVersionSdk() == AndroidVersions.SDK_L)
        	p = Pattern.compile("([A-Z_]+) #[0-9]+: Alarm\\{[a-f0-9]+ type [0-3] when [0-9]+ (.*)\\}");
        ...
        //Tinno:Cj	KK		type=0 whenElapsed=17994598 when=-1m41s268ms window=-1 repeatInterval=300000 count=0
        //Tinno:Shaobin	L	tag=*alarm*:android.intent.action.TIME_TICK
        //        			type=3 whenElapsed=+57s296ms when=+57s296ms
        //        			window=0 repeatInterval=0 count=0
        String props = item.getChild(0).getLine();
        if (brm.getAndroidVersionSdk() == AndroidVersions.SDK_L) {
        	props = item.getChild(0).getLine() +" "+ item.getChild(1).getLine() +" "+ item.getChild(2).getLine();
        	p = Pattern.compile("tag=(.*):(.*) type=(.?) .*whenElapsed=(.*) when=(.*) window=(.*) repeatInterval=(.*) count=(.*)");
        }else if (brm.getAndroidVersionSdk() >= AndroidVersions.SDK_KK)
        	p = Pattern.compile("type=(.?) .*whenElapsed=(.*) when=(.*) window=(.*) repeatInterval=(.*) count=(.*)");
        else
        ...
        if (brm.getAndroidVersionSdk() == AndroidVersions.SDK_L) {
        	alarm.whenS = m.group(5);
        	if (alarm.whenS.contains("ms"))
        		alarm.when = readTs(alarm.whenS);
        	else
        		alarm.when = 0;
	        alarm.repeat = Long.parseLong(m.group(7));
	        alarm.count = Long.parseLong(m.group(8));
        }else if (brm.getAndroidVersionSdk() >= AndroidVersions.SDK_KK)
        {
	        alarm.whenS = m.group(3);
	        alarm.when = readTs(alarm.whenS);
	        alarm.repeat = Long.parseLong(m.group(5));
	        alarm.count = Long.parseLong(m.group(6));        
	    }else {
        ...
        //Tinno:CJ		KK	operation=PendingIntent{42234178: PendingIntentRecord{422340a0 android broadcastIntent}}
        //Tinno:Shaobin	L	operation=PendingIntent{1e94f980: PendingIntentRecord{cafb7fa android broadcastIntent}}
        //no different, just care about read log lines
        String op;
        if (brm.getAndroidVersionSdk() == AndroidVersions.SDK_L)
        	op = item.getChild(3).getLine();
        else
        	op = item.getChild(1).getLine();
    ...
    public void generate(Module rep) {
        ...
        if (TopNode != null) genAlarmTop(br, mainCh);
    ...
    private Chapter genAlarmTop(BugReportModule br, Chapter mainCh) {
        Chapter ch = new Chapter(br.getContext(), "Top Alarms");
        mainCh.addChapter(ch);
	    PreText pre = new PreText(ch);
	    pre.addln(TopNode.getLine());
        for (DumpTree.Node item : TopNode) 
		pre.addln(item.getLine());
	return ch;
    }
