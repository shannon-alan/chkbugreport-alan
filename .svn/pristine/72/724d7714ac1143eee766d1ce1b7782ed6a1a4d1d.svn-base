...
public class SurfaceFlingerPlugin extends Plugin {
    ...
    private boolean scan(BugReportModule br, Section sec) {
        ...
        if (buff.startsWith("Sync configuration:")) {
            buff = sec.getLine(++line);
        }
        if (buff.startsWith("DispSync configuration:")) {
            buff = sec.getLine(++line);
        }
        ...
        if (found) {
            ...
            try {
                while (line < sec.getLineCount()) {
                    ...
                    if (buff.charAt(38) == '|') {
                        // pre 2.3
                        buffer.ptr = Integer.toString(Util.parseHex(buff, 0, 10, 0));
                        ...
                        if (m.matches()) {
                        	buffer.ptr = m.group(1);
//                            buffer.ptr = Integer.parseInt(m.group(1), 32);
    ...
    private int readRegion(Module br, Layer layer, String buff, Section sec, int line) {
        ...
        } else  if ("opaqueRegion".equals(type)) {
            reg = layer.regopaque;
        } else  if ("visibleRegion".equals(type)) {
            reg = layer.regvisible;
        } else  if ("coveredRegion".equals(type)) {
            reg = layer.regcovered;
        } else {
    ...
    private void readAttributes(BugReportModule br, Layer layer, String buff) {
        ...
        while (tok.hasMoreTokens()) {
            ...
            if ("layerStack".equals(key)||"z".equals(key)) {
                ...
                tok.nextToken(); // w
                tok.nextToken(); // h
            //Tinno:CJ add new attribute
            } else if ("isOpaque".equals(key)) {
            	layer.isOpaque = tok.nextInt();            
            } else {
    ...
    static class Layer {
	    ...
	    public Region regopaque = new Region("opaqueRegion");
	    public Region regvisible = new Region("visibleRegion");
	    public Region regcovered = new Region("coveredRegion");
